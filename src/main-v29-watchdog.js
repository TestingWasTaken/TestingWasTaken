...
function sendFailedState(pane, item, actualURL, targetURL) {
  item.exhausted = true;
  sendRecovery(pane, {
    active: true,
    failed: true,
...
function checkFollowers() {
  ...
  for (let pane = 2; pane <= Number(health.visiblePaneCount || 1); pane += 1) {
    ...
    if (item.exhausted) {
      sendFailedState(pane, item, actualURL, targetURL);
      continue;
    }
    ...
    item.settleUntil = now + (desiredURL ? 2200 : 1200);
    item.nextAttemptAt = item.settleUntil;
...
module.exports = { checkFollowers, challengeLike, MAX_ATTEMPTS };
