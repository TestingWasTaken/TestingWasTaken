...
const connectionScreen = $('#connection-screen');
const connectionTitle = $('#connection-title');
const connectionMessage = $('#connection-message');
const connectionProgressFill = $('#connection-progress-fill');
const connectionProgressLabel = $('#connection-progress-label');
...
function policyInputs() {
  return [policyNavigation, policyScrolling, policyTyping, policyClicks];
}

function normalizedPolicy(value = {}) {
  return {
    navigation: value.navigation === true,
    scrolling: value.scrolling === true,
    typing: value.typing === true,
    clicks: value.clicks === true,
  };
}
...
function draftFromState() {
  const policy = health?.followingEnabled
    ? normalizedPolicy(health.policy)
    : { navigation: false, scrolling: false, typing: false, clicks: false };
...
function progress(percent, message) {
  ...
  if (connectionActive) {
    connectionProgressLabel.textContent = `${value}%`;
    connectionProgressFill.style.width = `${value}%`;
    connectionMessage.textContent = message || 'Connecting…';
  }
}

function showConnection(mode) {
  connectionActive = true;
  connectionTitle.textContent = mode === 'tor' ? 'Connecting Multiple IPs' : 'Connecting Standard';
  connectionMessage.textContent = 'Preparing browser routes…';
  connectionProgressFill.style.width = '0%';
  connectionProgressLabel.textContent = '0%';
...
function routeText(index) {
  ...
  if (!result.ok) return result.error ? `Unavailable · ${result.error}` : 'Unavailable';
  return `${result.location || 'Location unavailable'} · ${result.ip}`;
}
...
function updateFacts() {
  ...
  if (verified.length) {
    const location = verified[0].location || verified[0].ip;
    $('#fact-routes').textContent = `${location} · ${verified.length}/${visible}`;
  } else {
    $('#fact-routes').textContent = 'IP/location not checked';
  }

  $('#fact-audio').textContent = `Sound: ${audioLabel(state.audioMode)}`;
  $('#fact-blocked').textContent = `${blocked} blocked`;
...
function rowStatus(row) {
  ...
  if (row.challenge) return { label: 'Skipped', tone: 'warn' };
...
function topologyKey() {
  return JSON.stringify({
    count: state?.screenCount,
    labels: state?.paneLabels,
    focused: state?.focusedPane,
    audio: state?.audioMode,
    ips: (state?.ips || []).slice(0, state?.screenCount || 0),
    health,
...
function renderTopology(force = false) {
  ...
  if (!force && signature === topologySignature) return;
  topologySignature = signature;

  topologyList.replaceChildren();
  const rows = health?.rows
    || Array.from({ length: state?.screenCount || 0 }, (_unused, index) => ({ paneNumber: index + 1 }));
  const fragment = document.createDocumentFragment();

  rows.forEach((row) => {
    const status = rowStatus(row);
    const item = document.createElement('div');
    item.className = `pane-row${state?.focusedPane === row.paneNumber ? ' is-focused' : ''}`;
    item.dataset.pane = String(row.paneNumber);

    const index = document.createElement('span');
    index.className = 'pane-index';
    index.textContent = String(row.paneNumber).padStart(2, '0');

    const copy = document.createElement('div');
    copy.className = 'pane-copy';
    const name = document.createElement('input');
    name.className = 'pane-name-input';
    name.value = state?.paneLabels?.[row.paneNumber - 1]
      || (row.paneNumber === 1 ? 'Main' : `Pane ${row.paneNumber}`);
    ...
    meta.textContent = `${status.label}${paneHasSound(row.paneNumber) ? ' · Sound' : ''}`;
    ...
    item.append(index, copy, route, actions);
...
async function openSettings() {
  if (busy || !state || settingsOpen()) return;
...
async function applySettings() {
  ...
  try {
    ...
    progress(34, `${draft.paneCount} panes · ${Math.round(draft.zoom * 100)}%`);

    if (draft.network !== state.networkMode) {
      showConnection(draft.network);
      progress(42, draft.network === 'tor' ? 'Starting Multiple IPs' : 'Starting Standard');
      const result = await window.conduit.setNetwork(draft.network);
      if (result?.ok === false) throw new Error(result.error || 'The connection could not be changed.');
      hideConnection();
    }
    ...
    if (draft.verifyRoutes) {
      progress(77, 'Checking IP addresses and locations');
...
async function finishBoot() {
  ...
  bootMessage.textContent = 'Opening Settings…';
  bootProgressFill.style.width = '100%';
  bootProgressLabel.textContent = '100%';

  if (!autoSettingsOpened) {
    autoSettingsOpened = true;
    await openSettings();
  }
...
function updateBoot() {
  ...
  let percent = state ? 25 : 8;
  let message = 'Starting browser sessions…';

  if (registered > 0) {
    percent = 28 + Math.round((Math.min(registered, visible) / Math.max(1, visible)) * 66);
  ...
  if (state && health && registered >= visible && !state.networkBusy) finishBoot();
}

bootFallbackTimer = setTimeout(() => finishBoot(), 9000);
...
adFilter.addEventListener('change', () => {
  if (!adFilter.checked && !window.confirm('Turn off the ad filter? Some sites may show pop-ups or tracking requests.')) {
    adFilter.checked = true;
  }
});

jujharLink.addEventListener('click', (event) => {
  event.preventDefault();
  window.conduit.openExternal(jujharLink.href);
});

topologyList.addEventListener('click', async (event) => {
  const button = event.target.closest('button[data-action]');
  const row = event.target.closest('.pane-row');
  if (!button || !row) return;

  const pane = Number(row.dataset.pane);
  if (button.dataset.action === 'focus') {
    await window.conduit.focusPane(state?.focusedPane === pane ? 0 : pane);
  }
...
document.addEventListener('keydown', (event) => {
  ...
  if (event.key.toLowerCase() === 'a' && document.activeElement === address) {
    event.preventDefault();
    address.select();
  }
  if (event.key.toLowerCase() === 'r') {
    event.preventDefault();
    event.shiftKey ? window.conduit.reloadAll() : window.conduit.reloadActive();
  }
  if (/^[1-8]$/.test(event.key)) {
    event.preventDefault();
    window.conduit.focusPane(Number(event.key));
  }
});

window.conduit.onState((next) => {
  state = next;
  if (!settingsOpen()) settingsDraft = null;
  render();
});

window.conduit.onLayout((next) => {
  layout = next?.labels || [];
  renderLabels();
});

window.conduit.onHealth((next) => {
  health = next;
  render();
});

window.conduit.onProgress((next) => {
  if (busy) progress(next?.percent, next?.message);
});
...
window.conduit.getWorkspace().then((initial) => {
  if (!state) {
    state = {
      ...initial,
      ips: initial.ips || [],
      adBlock: initial.adBlock || { enabled: true, totalBlocked: 0 },
      paneLabels: initial.paneLabels || [],
    };
  }
  render();
});

window.conduit.getHealth().then((initial) => {
  health = initial;
  render();
});
