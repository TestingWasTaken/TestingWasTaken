...
function sendDiagnostic(level, message) {
  const payload = { time: timestamp(), level, message: String(message || '') };
...
function resetRegistrationState() {
  screens.clear();
  challengedScreens.clear();
  alignmentLocks.clear();
  pendingActions.clear();
  latestControllerState = null;
  syncReadyReported = false;
  lastSampleSignature = '';
  fallbackScreenNumber = 0;
}

function screenNumberFromContents(contents) {
  const type = contents?.getType?.() || '';
  if (type !== 'browserView') return 0;
  if (fallbackScreenNumber >= MAX_SCREENS) return 0;
...
function smoothScrollContents(contents, state) {
  ...
  const script = `(() => {
    ...
    const key = '__conduitFollowerMotionV16';
    const motion = window[key] || {
      frame: 0,
      targetX: scrollX,
      targetY: scrollY,
      lastFrame: now,
    ...
    motion.targetX = ${JSON.stringify(xRatio)} * maxX;
    motion.targetY = ${JSON.stringify(yRatio)} * maxY;
    motion.lastTarget = now;
    window[key] = motion;

    if (motion.frame) return true;

    const tick = (time) => {
      const dt = Math.max(1, Math.min(40, time - motion.lastFrame));
      motion.lastFrame = time;

      const dx = motion.targetX - scrollX;
      const dy = motion.targetY - scrollY;
      const distance = Math.hypot(dx, dy);
      const response = distance > 1000 ? 46 : distance > 300 ? 58 : 72;
      const alpha = 1 - Math.exp(-dt / response);

      if (distance < 0.35 && time - motion.lastTarget > 70) {
        window.scrollTo(motion.targetX, motion.targetY);
        motion.frame = 0;
        return;
      }

      window.scrollTo(scrollX + (dx * alpha), scrollY + (dy * alpha));
...
function pushControllerPosition(contents, state, delays = [0]) {
  ...
  if (!registeredContents.has(contents)) {
    ...
    contents.on('did-stop-loading', () => {
      ...
      if (syncEnabled && latestControllerState && !challengedScreens.has(screenNumber)) {
        pushControllerPosition(contents, latestControllerState, [30, 180, 480]);
      }
    });
  }
...
app.on('browser-window-created', (_event, window) => {
  ...
  window.once('closed', resetRegistrationState);
});
...
const scrollSampler = setInterval(sampleControllerPosition, 34);
scrollSampler.unref?.();
...
ipcMain.handle('open-external-v16', async (_event, value) => {
  try {
    const url = new URL(String(value || ''));
...
ipcMain.on('controller-action-v12', (event, action) => {
  ...
  // Window scrolling is handled by the continuous sampler and follower spring.
  ...
  const actionId = `v16-${++actionSequence}`;
  pendingActions.set(actionId, { expected: followers.length, received: 0, failed: 0 });
